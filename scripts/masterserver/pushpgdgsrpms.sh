...
#for i in `ls -d 9.1 9.0 8.4 8.3 9.2`
for i in `ls -d 9.1`
do
	cd $i
...
# 		for k in `ls -d fedora-16* rhel-*`
		...
		do
			...
			rsync -ave ssh  --delete /media/disk/RPMS/srpms-web/$i/$j/$k  devrim@www.gunduz.org:/home/community/postgresqlrpms.org/public_html/srpms/$i/$j/
			cd ..
		done
		cd ..
	done
