...
do
	...
	then
		...
		rsync --checksum -ave ssh --delete $RPM_DIR/ yumupload@yum.postgresql.org:zypp/zypp/$packageSyncVersion/$osdistro/$os.$osminversion-$osarch
	fi
	...
	aws s3 sync --delete $SRPM_DIR s3://zypp-srpms.postgresql.org20250618120322107700000001/srpms/$packageSyncVersion/$osdistro/$os.$osminversion-$osarch/repodata/ --exclude "*.html"
	...
	aws s3 sync --delete $DEBUG_RPM_DIR s3://zypp-debuginfo.postgresql.org20250312201116651400000002/debug/$packageSyncVersion/$osdistro/$os.$osminversion-$osarch/repodata/ --exclude "*.html"
