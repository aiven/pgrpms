...
aws s3 sync --delete $COMMON_SRPM_DIR/repodata/ s3://zypp-srpms.postgresql.org20250618120322107700000001/srpms/common/$packageSyncVersion/$osdistro/$os.$osminversion-$osarch/repodata/ --exclude "*.html"
aws cloudfront create-invalidation --distribution-id $CF_SRPM_DISTRO_ID --path /srpms/common/$packageSyncVersion/$osdistro/$os.$osminversion-$osarch/repodata/*

# Sync debug* RPMs to S3 bucket:
aws s3 sync $COMMON_DEBUG_RPM_DIR s3://zypp-debuginfo.postgresql.org20250312201116651400000002/debug/common/$osdistro/$os.$osminversion-$osarch/ --exclude "*.html" --exclude "repodata"
aws s3 sync --delete $COMMON_DEBUG_RPM_DIR/repodata/ s3://zypp-debuginfo.postgresql.org20250312201116651400000002/debug/common/$packageSyncVersion/$osdistro/$os.$osminversion-$osarch/repodata/ --exclude "*.html"
aws cloudfront create-invalidation --distribution-id $CF_DEBUG_DISTRO_ID --path /debug/common/$packageSyncVersion/$osdistro/$os.$osminversion-$osarch/repodata/*
