...
rsync -av --delete --stats /var/lib/pgsql/rpm/RPMS/$osarch/ /var/lib/pgsql/rpm/RPMS/noarch/ $RPM_DIR
rsync -av --delete --stats /var/lib/pgsql/rpm/SRPMS/ /var/lib/pgsql/rpm/ALLSRPMS

# Now, create repo for RPMs and SRPMS:

createrepo --checksum=sha -g /usr/local/etc/postgresqldbserver-$pgrelease.xml -d --update $RPM_DIR && repoview -u "http://yum.postgresql.org/$pgrelease/$osdistro/$os-$osarch/" -o repoview/ -t "PostgreSQL PGDG $pgrelease RPMs" $RPM_DIR/
createrepo --checksum=sha --update $SRPM_DIR && repoview -u "http://yum.postgresql.org/srpms/$pgrelease/$osdistro/$os-$osarch/" -o repoview/ -t "PostgreSQL PGDG $pgrelease RPMs" $SRPM_DIR/
