...
repo --name=a-base    --baseurl=ftp://ftp.linux.org.tr/centos/6/os/$basearch
repo --name=a-updates --baseurl=ftp://ftp.linux.org.tr/centos/6/updates/$basearch
repo --name=a-extras  --baseurl=ftp://ftp.linux.org.tr/centos/6/extras/$basearch
repo --name=a-live    --baseurl=http://www.nanotechnologies.qc.ca/propos/linux/centos-live/$basearch/live
repo --name=a-epel    --baseurl=ftp://ftp.linux.org.tr/epel/6/$basearch

# PGDG RPM Repository
repo --name=pgdg92  --baseurl=http://yum.postgresql.org/9.2/redhat/rhel-6-i386
...
# add set no password for postgres user. It is already created with postgresql92-server RPM.
...
chkconfig postgresql-9.2 on 2>/dev/null
...
su -l postgres -c "/usr/pgsql-9.2/bin/initdb -D /var/lib/pgsql/9.2/data"
service postgresql-9.2 start

# Create pagila database and load pagila data into it:
su -l postgres -c "createdb pagila"
su -l postgres -c "psql pagila -f /usr/share/pagila92/pagila-schema.sql"
su -l postgres -c "psql pagila -f /usr/share/pagila92/pagila-data.sql"
