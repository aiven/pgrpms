...
# List of repositories. We want to stick to x86_64 builds only.

# These are the public repositories
repo --name=released --mirrorlist=http://mirrors.fedoraproject.org/mirrorlist?repo=fedora-16&arch=x86_64
repo --name=updates --mirrorlist=http://mirrors.fedoraproject.org/mirrorlist?repo=updates-released-f16&arch=x86_64

# PGDG RPM Repository
repo --name=pgdg91  --baseurl=http://yum.pgrpms.org/9.1/fedora/fedora-16-x86_64
...
# add set no password for postgres user. It is already created with postgresql91-server RPM.
...
systemctl enable postgresql-9.1.service  2>/dev/null
...
su -l postgres -c "/usr/pgsql-9.1/bin/initdb -D /var/lib/pgsql/9.1/data"
