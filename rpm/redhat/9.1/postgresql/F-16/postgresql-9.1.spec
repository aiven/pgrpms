...
%description server
PostgreSQL is an advanced Object-Relational database management system (DBMS).
The postgresql91-server package contains the programs needed to create
...
%endif
Obsoletes:	postgresql91-pl
Provides:	postgresql-plperl

%description plperl
The postgresql91-plperl package contains the PL/Perl procedural language,
...
The postgresql91-plpython package contains the PL/Python procedural language,
which is an extension to the PostgreSQL database server.
...
if [ $1 -eq 0 ] ; then
	...
	/bin/systemctl --no-reload disable postgresql-9.1.service >/dev/null 2>&1 || :
	/bin/systemctl stop postgresql-9.1.service >/dev/null 2>&1 || :
fi
...
if [ $1 -ge 1 ] ; then
	...
	/bin/systemctl try-restart postgresql-9.1.service >/dev/null 2>&1 || :
fi
