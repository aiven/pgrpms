...
Requires:	postgresql-server >= 8.4, libmemcached >= 0.39
...
install -m 755 %{name}.so %{buildroot}%{pginstdir}/lib/%{name}-%{version}.so
%{__ln_s} %{pginstdir}/lib/%{name}-%{version}.so %{buildroot}%{pginstdir}/lib/%{name}.so
